...
function init(){
    $("#semestre_form").on("submit",function(e){
        guardaryeditar(e);
    });
...
function nuevo(){
    $('#titulo_modal').html('Nuevo Semestre');
    $('#semestre_form')[0].reset();
    $('#modalcrearSemestre').modal('show');
}
...
function editar(seme_id){
    $.post("/Maie/controller/semestre.php?opc=mostrar",{seme_id:seme_id},function (data){
        ...
        $('#seme_nombre').val(data.seme_nombre);
    });
    $('#titulo_modal').html('Editar Semestre');
    $('#modalcrearSemestre').modal('show');
}

function eliminar(seme_id){
    Swal.fire({
        title: 'Eliminar!',
