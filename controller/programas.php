    ...
    switch($_GET["opc"]){
        ...
        case "guardaryeditar":
            if(empty($_POST["prog_id"])){
                $programas->insert_programas($_POST["codigo"],$_POST["descripcion"],$_POST["cen_id"]);     
            }else {
                $programas->update_programas($_POST["prog_id"],$_POST["codigo"],$_POST["descripcion"],$_POST["cen_id"]);
            }
            break;
        ...
        case "listar":
            ...
            foreach($datos as $row){
                ...
                $sub_array[] = '<button type="button" onClick="editar('.$row["prog_id"].');"  id="'.$row["prog_id"].'" class="btn btn-outline-warning btn-sm ladda-button"><i class="fa fa-edit"></i></button>';
                $sub_array[] = '<button type="button" onClick="eliminar('.$row["prog_id"].');"  id="'.$row["prog_id"].'" class="btn btn-outline-danger btn-sm ladda-button"><i class="fa fa-trash"></i></button>';
                $data[] = $sub_array;
            }
        ...
        case "combo":
            ...
            if(is_array($datos)==true and count($datos)>0){
                foreach($datos as $row)
                {
                    $html.= "<option value='".$row['prog_id']."'>".$row['codigo']."-".$row['descripcion']."</option>";
                }
                echo $html;
            }
            break;
